...
def test_03_rbac_list_users():
    ...
    assert r.status_code == 401 or r.status_code == 403  # butuh header lengkap / forbidden
...
def test_04_get_self_and_forbidden_other():
    # staff A boleh lihat dirinya
    r = client.get(f"/users/{USER_ID}", headers=STAFF_A)
    # jika USER_ID bukan u-a, ini semestinya 403; maka kita adjust: coba admin ambil id dulu
    if r.status_code in (401,403,404):
        # ambil id pertama via admin, lalu minta sebagai owner
        rows = client.get("/users", headers=ADMIN).json()
        uid0 = rows[0]["id"]
        # minta sebagai owner (ubah header id jadi sama dengan uid0)
        r = client.get(f"/users/{uid0}", headers={"X-User-Id": uid0, "X-User-Role": "staff"})
    assert r.status_code == 200
    # staff B tidak boleh lihat profil staff A
    uid = r.json()["id"]
    r2 = client.get(f"/users/{uid}", headers=STAFF_B)
    assert r2.status_code == 403

def test_05_update_self_and_no_password_leak():
    # ambil satu id via admin
    rows = client.get("/users", headers=ADMIN).json()
    uid = rows[0]["id"]
...
def test_06_change_password_by_owner_and_admin_override():
    ...
    # ganti password salah current -> 403
    r = client.post(
